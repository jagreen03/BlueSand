...
$excludeDirRegex = [regex]::new($cfg.exclude_dir_regex, [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)
$excludeFileRegex = if ([string]::IsNullOrWhiteSpace($cfg.exclude_file_regex)) { $null }
                    else { [regex]::new($cfg.exclude_file_regex, [System.Text.RegularExpressions.RegexOptions]::IgnoreCase) }
...
foreach ($rootPath in $includePaths) {
  ...
  Where-Object {
    ...
    $dirOK  = -not $excludeDirRegex.IsMatch($_.FullName)
    $fileOK = ($excludeFileRegex -eq $null) -or (-not $excludeFileRegex.IsMatch($_.FullName))
